...
def main():
    ...
    features = extractColorHistogramFeatures([image])
    ...
    result = f'O valor sorteado para a maquina foi: {random_choice}'

    print(result)
    # Imagem escolhida
    escolha_maquina = random_choice

    # Comparação com a imagem reconhecida
    if escolha_maquina == 'rock':
        if predicted_class == 'rock':
            result = "Empate"
        elif predicted_class == 'scissor':
            result = "Você Perdeu!"
        else:
            result = "Você Ganhou!"
    elif escolha_maquina == 'paper':
        if predicted_class == 'rock':
            result = "Você Perdeu!"
        elif predicted_class == 'scissor':
            result = "Você Ganhou!"
...
def getData(path):
    ...
    if os.path.exists(path):
        ...
        bar = Bar('[INFO] Getting images and labels...', max=len(fileList), suffix='%(index)d/%(max)d Duration:%(elapsed)ds')
...
def extractColorHistogramFeatures(images):
    bar = Bar('[INFO] Extracting color histogram features...', max=len(images), suffix='%(index)d/%(max)d  Duration:%(elapsed)ds')
    featuresList = []
